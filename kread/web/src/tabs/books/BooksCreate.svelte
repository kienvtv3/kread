<script>
  ...
  let downloadName = $state('test.kp')
  ...
  let lastKpFile = $state(null)
  let pushing = $state(false)
  let pushStatus = $state('')

  async function handlePush() {
    if (!lastKpFile || !connection.connected) return
    pushing = true
    pushStatus = 'Pushing...'
    try {
      await connection.pushFile('test.kp', lastKpFile, (received, total) => {
        const pct = Math.round(received / total * 100)
        pushStatus = `Pushing... ${pct}%`
      })
      pushStatus = 'Pushed! Reboot device to display.'
    } catch (err) {
      pushStatus = `Push failed: ${err.message}`
  ...
  function showResult(result) {
    // Write quantized pixels to emulator screen
    screenStore.writeQuantized(result.quantized, result.pageWidth, result.pageHeight)

    // Store kp file for push
    lastKpFile = result.kpFile
  ...
  async function handleConvert() {
    ...
    try {
      ...
      downloadName = (booksStore.epubName || 'book') + '.kp'
      showResult(result)
    } catch (err) {
...
<div class="books-create">
  ...
  <div class="actions">
    ...
    <button class="primary-btn"
      disabled={!connection.connected || !lastKpFile || pushing}
      onclick={handlePush}
    >
      {pushing ? pushStatus : 'Push to Device'}
    </button>
    {#if pushStatus && !pushing}
      <div class="push-status">{pushStatus}</div>
    {/if}
  </div>
</div>
