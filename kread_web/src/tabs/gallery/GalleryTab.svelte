<script>
  ...
  let downloadName = $state('page.kpage')
  ...
  function convertAndDisplay(img) {
    ...
    requestAnimationFrame(() => {
      try {
        ...
        const { bw, gs, width, height } = pixelsToLandscapePlanes(pixels, SCREEN_W, SCREEN_H)
        const kpageFile = encodePlanes(bw, gs, width, height)
        outputBytes = kpageFile.length
        cleanupDownload()
        downloadUrl = URL.createObjectURL(new Blob([kpageFile], { type: 'application/octet-stream' }))
        downloadName = `${imageName || 'image'}.kpage`
